...
#define IMPORT_PATTERN // Need only MAXPATLEN ; maybe should decouple
...
extern char *ask_string(const char *prompt, char *buf, size_t bufsz);
...
/**
 * @brief Execute a compiled move plan.
 *
 * Given a set of { from->to } pairs, validate, analyze, and perform
 *
 * @param mmv
 *
 * Somehow -- we do not care how -- a set of { from -> to }
 * filename pairs has been read in or generated by patterns,
 * and produced a compiled move plan.
 *
 * Execute that move plan.  Do all the heavy lifting.
...
/**
 * @brief Add a single { from->to } pair of filenames to the set of |REP|s.
 *
 * @param mmv
...
int
mmv_add_1_fname_pair(mmv_t *mmv, const char *src_fname, const char *dst_fname)
{
    int err;

    // XXX Ensure that repeated calls to mmv_add_1_fname_pair()
...
/*
 * Add a list of  { from -> to } filename pairs to the given mmv object.
 */

int
mmv_add_fname_pairs(mmv_t *mmv, size_t filec, char **filev)
{
    size_t i;
    int err;

    if (! even(filec)) {
        eprintf("There are %zu filenames", filec);
        eprintf(", which is not an even number.\n");
        eprintf("Filenames must come in { from -> to } pairs.\n");
        return (EINVAL);
    }

    for (i = 0; i < filec; i += 2) {
        err = mmv_add_1_fname_pair(mmv, filev[i], filev[i + 1]);
        if (err) {
            return (err);
        }
    }
...
int
mmv_pairs_argv(mmv_t *mmv, size_t filec, char **filev)
{
    int err;

    err = mmv_add_fname_pairs(mmv, filec, filev);
    if (err) {
        return (err);
    }
    return (0);
